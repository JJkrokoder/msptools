...
class TestParticles:
    ...
    def test_add_one_particle(self):
        particles = msp.Particles(xp = np)
        positions = np.array([[0.0, 0.0, 0.0]])
        polarizabilities = np.array([1.0*np.eye(3)])
        ...
        assert len(particles.polarizabilities) == 1, "There should be one polarizability in the system"
    
    def test_add_particles(self):
        particles = msp.Particles(xp = np)
        positions = np.array([[0.0, 0.0, 0.0], [1.0, 1.0, 1.0]])
        polarizabilities = np.array([1.0*np.eye(3), 2.0*np.eye(3)])
        particles.add_particles(positions, polarizabilities)
        assert len(particles.positions) == 2, "There should be two particles in the system"
