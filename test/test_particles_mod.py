...
def test_initialize_single_type():
    ...
    particles = msp.ParticleData(types=custom_type)
    assert len(particles.types) == 1, "The system should have one particle type"

class TestParticleData:
    my_type = msp.SphereType(radius=1.0, material="mat1")

    def test_clean_positions(self):
        particles = msp.ParticleData(self.my_type)
    ...
    def test_add_1particle_formats(self):
        particles = msp.ParticleData(self.my_type)
    ...
    def test_add_1particle_type_assignment(self):
        particles = msp.ParticleData(self.my_type)
        ...
        particle_2_radius = particles.types[particles.type_assignments[2]].radius

        assert particle_2_radius == 1.0, "The radius of the type assigned to the third particle is incorrect"
            

    def test_add_2particles_formats(self):
        particles = msp.ParticleData(self.my_type)
