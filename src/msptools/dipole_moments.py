...
def calculate_dipole_moments_linear(
    polarizability: np.ndarray | complex | List[complex] | float | List[float],
    ...
    if isinstance(polarizability, (complex, float, int)):
        ...
        number_of_polarizabilities = (
            len(polarizability)
            if isinstance(polarizability, list)
            else polarizability.shape[0]
        )
        if number_of_polarizabilities != electric_field.shape[0]:
            raise ValueError(
                "Polarizability and electric field must have the same number of elements."
            )
        dipole_moments = (
            np.array(polarizability[:number_of_polarizabilities])
            * electric_field[:number_of_polarizabilities, :]
        )

    else:
        raise TypeError(
            "Polarizability must be a complex number, float, int, list, or numpy array."
        )

    return dipole_moments


def polarizability_to_matrix(
    polarizability, num_particles: int, dimensions: int
) -> np.ndarray:
    """
    ...
    if isinstance(polarizability, (complex, float, int)):
        ...
        return np.diag(
            [polarizability[i] for i in range(num_particles) for _ in range(dimensions)]
        )
