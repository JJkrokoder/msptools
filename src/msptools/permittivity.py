...
def permittivity_Drude(
    ...
    """
    epsilon = epsilon_inf - (plasma_frequency**2) / (
        frequency**2 + 1j * frequency * collision_frequency
    )
    return epsilon
...
def permittivity_ridx(frequency: float | np.ndarray, material: str) -> complex:
    ...
    if isinstance(frequency, np.ndarray):
        ...
        epsilon = np.array(
            [Material.get_epsilon(wavelength_nm=w) for w in wavelength_nm]
        )
    else:
        wavelength_nm = eV_to_nm(frequency)
