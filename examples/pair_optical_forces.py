...
plt.scatter(x, forces_s_analytical, label='s-polarization analytical', color='cyan', s=10, marker='x')
plt.scatter(x, forces_p_analytical, label='p-polarization analytical', color='orange', s=10, marker='x')
#plt.yscale('log')
plt.xlabel('Distance (nm)')
plt.ylabel('Optical Force (a.u.)')
plt.title(f'Optical Forces between two Au NPs ({radius_nm} nm radius) at {wavelength_nm} nm wavelength')
plt.legend()
...
forces_p_far_analytical = np.abs(polarizability)**2 * np.cos(k_m * x_far) * k_m**2 / x_far**2/(4*np.pi)
forces_s_far_analytical = -np.abs(polarizability)**2 * np.sin(k_m * x_far) * k_m**3 / (2*x_far)/(4*np.pi)

plt.figure(figsize=(8,6))
plt.plot(x_far, forces_s_far[:,1,0], label='s-polarization (numerical, far)', color='blue')
plt.plot(x_far, forces_p_far[:,1,0], label='p-polarization (numerical, far)', color='red')
plt.plot(x_far, forces_s_far_analytical, label='s-polarization analytical (far)', color='cyan', linestyle='--')
plt.plot(x_far, forces_p_far_analytical, label='p-polarization analytical (far)', color='orange', linestyle='--')
plt.xlabel('Distance (nm)')
plt.ylabel('Optical Force (a.u.)')
plt.title(f'Far-field Optical Forces between two Au NPs ({radius_nm} nm radius) at {wavelength_nm} nm')
plt.legend()
plt.grid(True, which="both", ls="--")
plt.show()
